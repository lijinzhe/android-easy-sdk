 ...
 * 图片压缩工具类
 * <p/>
 * 用于对大图片，例如：照片，屏幕截屏等图片进行无损压缩，从而方便进行图片的上传操作
 * <p/>
 * Created by Lee on 16/8/7.
 */
public class UECompress {
    public static final int FIRST_GEAR = 1;
    public static final int THIRD_GEAR = 3;

    public static String DEFAULT_DISK_CACHE_DIR = "ue_bitmap_compress_disk_cache";

    private static volatile UECompress INSTANCE;
    ...
    private UECompress(File cacheDir) {
        mCacheDir = cacheDir;
    }

    public static UECompress with(Context context) {
        if (INSTANCE == null) {
            File compressCacheDir = null;
            File cacheDir = context.getCacheDir();
            //内部缓存空间
            if (cacheDir != null) {
                File result = new File(cacheDir, DEFAULT_DISK_CACHE_DIR);
                if (result.mkdirs() || (result.exists() && result.isDirectory())) {
                    compressCacheDir = result;
                }
            }
            //如果内部缓存空间创建失败，则使用外部缓存空间
            ...
            INSTANCE = new UECompress(compressCacheDir);
        }
        return INSTANCE;
    }
     ...
     * @return
     */
    public UECompress from(File file) {
        mFile = file;
        return this;
    }
     ...
     * @return
     */
    public UECompress putGear(int gear) {
        this.gear = gear;
        return this;
    }
     ...
     * @return
     */
    public UECompress setCompressListener(OnCompressListener listener) {
    ...
    public UECompress compress() {
        ...
        else if (gear == UECompress.THIRD_GEAR)
