...
public class CropTransfer implements Transformation<Bitmap> {
    ...
    public CropTransfer(Context context) {
        this(Glide.get(context).getBitmapPool());
    }

    public CropTransfer(BitmapPool pool) {
        this(pool, 0, 0);
    }

    public CropTransfer(Context context, int width, int height) {
        this(Glide.get(context).getBitmapPool(), width, height);
    }

    public CropTransfer(BitmapPool pool, int width, int height) {
        this(pool, width, height, CropType.CENTER);
    }

    public CropTransfer(Context context, int width, int height, CropType cropType) {
        this(Glide.get(context).getBitmapPool(), width, height, cropType);
    }

    public CropTransfer(BitmapPool pool, int width, int height, CropType cropType) {
        mBitmapPool = pool;
        mWidth = width;
        mHeight = height;
        mCropType = cropType;
